/**
 * functions/api/extract.js - Cloudflare Pages Function
 * OpenRouter AI 견적서 자동 추출 API 엔드포인트
 */
  ...
  try {
    ...
    if (!apiKey || apiKey.trim() === '') {
      return new Response(JSON.stringify({
        ...
        message: 'Cloudflare 환경변수에 OPENROUTER_API_KEY가 설정되지 않았습니다. Cloudflare Pages 대시보드(Settings > Variables and Secrets)에 키를 등록해 주세요.'
      }), {
        status: 500,
    ...
    const selectedModel = model || env.DEFAULT_MODEL || 'google/gemini-2.5-flash';
...
반드시 다른 부가적인 설명 없이 오직 유효한 순수 JSON만 응답하세요. JSON 스키마 형식:
{
  ...
  "note": "전체 특이사항 또는 결제조건"
    ...
    if (!openRouterResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: 'AI_API_ERROR', message: msg }), {
    ...
    if (!content) {
      return new Response(JSON.stringify({ error: 'EMPTY_AI_RESPONSE', message: 'AI 모델로부터 응답을 받지 못했습니다.' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json; charset=utf-8' }
      });
    }
